...
if (length(args) < 4) {
  stop("Usage: SCRS_qc.R <input_folder> <output_dir> <metadata> <prefix>", call.=FALSE)
}
...
#Add No_Cell, Group, Filter and other meta information
raw.data<-cbind(No_Cell=seq(1:nrow(raw.data)), Group=Group, 
    Filter=factor(x='Pass',levels=c("Pass","Fail")), raw.data)
...
spc_min <- 15
data_bad <- data_hyperSpec[data_hyperSpec$data_hyperSpec_spc_min <= spc_min]
if (length(data_bad)>0) {
  data_hyperSpec[data_hyperSpec$data_hyperSpec_spc_min <= spc_min,'Filter'] <- 'Fail'
}
...
factors <- 1/apply(data_baseline_zero_hyperSpec[,,2900~3050],1,max) #normalize based on mean value of C-H peak
data_baseline_normalization <- sweep(data_baseline_zero_hyperSpec,1,factors,"*")
data_hyperSpec_spc_mean <- apply(data_baseline_normalization[,,400~550],1,mean)
data_hyperSpec_spc_sd <- apply(data_baseline_normalization[,,400~550],1,sd)
data_baseline_normalization$data_hyperSpec_spc_mean <- data_hyperSpec_spc_mean
data_baseline_normalization$data_hyperSpec_spc_sd <- data_hyperSpec_spc_sd
mean_fail_ids <- data_baseline_normalization[data_baseline_normalization$data_hyperSpec_spc_mean>0.5,"ID_Cell"]
sd_fail_ids <- data_baseline_normalization[data_baseline_normalization$data_hyperSpec_spc_sd>0.3,"ID_Cell"]
fail_ids <- unique(c(mean_fail_ids,sd_fail_ids))
str(mean_fail_ids)
str(sd_fail_ids)
str(fail_ids)
q()
