...
tabItem(
  ...
  fluidRow(
    box(
      ...
      fluidRow(
        column(2, numericInput("trim_min", "Min", min = 0, max = 4000,
                               step = 1, value = 400, width = "100%")),
        column(
          8,
        ...
        column(2, numericInput("trim_max", "Max", min = 0, max = 4000,
                               step = 1, value = 3400, width = "100%"))
      ),
      fluidRow(
    ...
    box(
      ...
      column(
        12,
        p("1. Select the wavelength region to keep,
          only one region is supported."),
        p("2. The wavelength region can be broader than your spectra,
          but should not be too narrow."),
        p("3. Choose the target spectra collection
          from the dropdown selector."),
        p("4. Row of the spectra table (bottom left) can be selected to
          visual the spectrum (bottom right).")
      )
    )
  ),
  fluidRow(
