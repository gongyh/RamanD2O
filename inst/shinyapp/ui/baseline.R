...
tabItem(
  ...
  fluidRow(
    box(
      ...
      column(
        ...
        fluidRow(
          ...
          column(6,
                 withBusyIndicatorUI(actionButton("baseline",
                                                  "Baseline",
                                                  class = "btn-success")),
                 class = "top25")
        ),
        fluidRow(
          ...
          conditionalPanel(
            ...
            conditionalPanel(
              ...
              column(3, numericInput("polyfit_order", "Order",
                                     1, min = 0, max = 15, step = 1))
            ),
            conditionalPanel(
              condition = "input.polyfit_custom == true",
              column(1, actionButton("polyfit_custom_plus", "",
                                     icon = icon("plus")), class = "top25"),
              column(1, actionButton("polyfit_custom_minus", "",
                                     icon = icon("minus")), class = "top25"),
              column(1)
            ),
            column(3, checkboxInput("polyfit_custom", "Custom range",
                                    value = FALSE), class = "top25")
          ),
          conditionalPanel(
        ...
        conditionalPanel(
          condition = paste0("input.polyfit_custom ==",
                             " true & input.select_baseline == 'polyfit'"),
    ...
    box(
      ...
      column(
        12,
        p("1. Choose the target spectra collection
          from the dropdown selector."),
        p("2. Choose baseline method, currently support polyfit and ALS."),
        p("3. Negative values can be set to zero, or pull the
          whole spectrum up, or leave as it is."),
  ...
  fluidRow(
    box(
      ...
      column(12,
             DTOutput("baselined_table", height = "400px") %>% withSpinner())
    ),
    box(
