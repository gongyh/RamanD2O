...
observeEvent(input$unzip, {
  withBusyIndicatorServer("unzip", {
    if (!is.null(isolate(input$scrs_file$datapath))) {
      ...
      files <- utils::unzip(isolate(input$scrs_file$datapath),
                            list = FALSE, exdir = tempdir())
      ...
      if (isolate(input$align)) {
        ...
        for (filename in txtfiles) {
          ...
          if (!identical(shift, cur_shift)) {
            remove_modal_progress()
            toastr_error("Wavelength differences found, please double check!",
                         position = "top-center")
            return()
          }
          dt <- content$V2
      ...
      remove_modal_progress()
      toastr_success(paste0("Load ",
                            length(txtfiles), " spectrum files."),
                     position = "top-center")
    } else {
